...
export async function routedAnswer(env: Env, system: string, prompt: string, media: GeminiInput[], thinking: ThinkingLevel, now: number): Promise<AnswerResult> {
  ...
  try {
    ...
    if (intent) return { ...result, text: await searchEventIntent(env, intent) };
